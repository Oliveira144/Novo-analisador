...
def detectar_padrao(seq):
    ...
    # Converte lista para string para facilitar checagens de sequência
    seq_str = ''.join(seq)
    ...
    elif len(seq) >= 6 and seq_str[-3:] == seq_str[-6:-3]:
        nivel = 2
    ...
    # Alternância forçada
    elif len(seq_str) >= 3 and (seq_str[-3:] == "🔴🔵🔴" or seq_str[-3:] == "🔵🔴🔵"):
        nivel = 5
        padrao = "Alternância Forçada"
    # Quebra pós-empate
    elif "🟡" in seq and (seq_str.endswith("🟡🔴") or seq_str.endswith("🟡🔵")):
        nivel = 6
...
def analisar_curto_medio(h):
    ...
    # Recomendação direta
    if pred == "🔴":
        recomendacao = "Aposte na Casa"
    elif pred == "🔵":
        recomendacao = "Aposte no Visitante"
    else:
        recomendacao = "Empate provável"
